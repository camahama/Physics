...
export class PrecessionModel {
  private state: PrecessionState;
  ...
  step(deltaSeconds: number): PrecessionState {
  ...
  private advanceDynamics(deltaSeconds: number): void {
    ...
    this.state.precessionAngle += precessionRate * deltaSeconds;
  }

  private recomputeDerivedState(): void {
    const spinSign = this.state.spinRate >= 0 ? 1 : -1;
    const safeSpinRate =
      spinSign * Math.max(Math.abs(this.state.spinRate), MIN_SPIN);
    const precessionRate = this.computePrecessionRate(safeSpinRate);
    const yaw = this.state.precessionAngle;
    const spin = this.state.spinAngle;

    const axisOrientation = new Quaternion().setFromEuler(
      new Euler(this.state.tiltAngle, yaw, 0, "YXZ")
    );
    ...
    const bodyAxis = new Vector3(0, 1, 0)
      .applyQuaternion(axisOrientation)
      .normalize();
  ...
  private computePrecessionRate(safeSpinRate: number): number {
    ...
    const limitedRate =
      Math.sign(steadyRate) *
      Math.min(Math.abs(steadyRate), PRECESSION_LIMIT);
